...
**These figures predate §5.2's prohibition and are now generous by orders of magnitude.** A conformant circuit carries a DCUtR negotiation and closes; 8MB and 120 seconds bound a *misbehaving* peer rather than a working one. An implementation is free to set both far lower — the negotiation's own cost is the honest floor — and doing so converts a rule that is otherwise only enforceable by inspection into one a relay enforces by itself.
