...
**Document status:** v1.7 — stable. §5.4 forbids a relay announcing an address no remote peer could dial. Loopback is obvious; **IPv6 link-local** is the one that reached a deployment, where it entered every reservation reply, became a circuit address clients advertised, rode into invites, and produced a repeating handshake timeout naming a peer id the network no longer used — against a relay working perfectly otherwise, and with no way for an ordinary member to stop it. Private addresses stay announceable, since a relay on a member's LAN is genuinely reachable. Previously v1.6 — stable. §5.3 gains a requirement on the **client** side of a relay's limits: a reservation that is refused MUST be re-asked with a growing interval. Every limit in that section is a rate, and the reference limiter refills about one reservation per two minutes per peer — so a client re-asking on a fixed twenty-second interval spends six times what it earns, empties the bucket, and is then refused indefinitely while still asking at the same rate. The retry sustains the failure it is trying to clear, on every affected member at once, while ordinary connections keep working. Found in a real deployment rather than by review, which is also why it is stated here rather than left as client policy: a section that specifies rates on one side and says nothing about the other has specified half a mechanism, the same way §5.1's acknowledgement did in v1.5. Previously v1.5 — stable. §5.1's direct-delivery carrier gains a fourth obligation: the acknowledgement **MUST be reported to the sender**. Saying that an acknowledgement existed turned out not to say that anybody could see one — the reference implementation received the response and had no arm for it, so it was dropped a layer below the consumer that needed it, leaving a sender to choose between re-sending forever and losing the payload after one attempt. That is the third time this section's mechanism has been completed by the first consumer to try to use it, after §1.2's proof and §5.6's invite bytes, and the pattern is now named: a mechanism specified from the receiving end is specified halfway. Previously v1.4 — stable. §1.2 requires the common-ownership proof to have a **serialized form**, because "shared voluntarily" is half the mechanism and a proof nobody can hand over is a proof nobody can voluntarily make — the same gap §5.6 records for invites, found the same way, by something finally trying to send one. Decoding one must verify both signatures, and a recipient must additionally check *which* pair it names: the interesting forgery is two genuine signatures over a true statement about somebody else. Previously v1.3 — stable. §5.1 gains **direct member-to-member delivery**, `/intranet/direct/1.0.0`: a carrier for handing one member a payload rather than publishing it, requested by the Chat Application Spec as `/chat/dm-invite/1.0.0` and landed generically — a namespace, a kind and an opaque payload — because §0 states the platform must not be shaped around one application. That is the third request answered this way, after §2.7.2's application entry and §2.6.2's policy map, and the pattern is now stated rather than rediscovered. It carries three obligations an implementation owes and a consuming spec cannot add later: the payload is signed over sender, namespace, kind and content together; the claimed sender is checked against the connection it arrived on, which a signature cannot do for itself because a signature travels; and delivery is metered per identity rather than per peer id. Nothing is stored by either party, and an implementation may not queue undelivered payloads on a third node. Previously v1.2 — stable. §1.1–§1.3 admit **two identity models** rather than one: a master seed from which per-network keypairs are derived, and independent per-network seeds. §1.2's properties are now stated as requirements on the *result* rather than as consequences of derivation, since independent generation satisfies all three — unlinkability by construction, which is the weaker assumption. §1.1 states the trade in both directions and what recovery actually costs under each, including that a phrase alone restores nothing under either. §1.3's enrollment and device-authority clauses no longer assume a master seed exists. This closes a divergence that had been real for some time: an implementation had shipped the second model, and the specification named only the first. Previously v1.1 — stable. §2.5.1 adds **self-removal**: a membership removal naming its own author needs no capability, because the protocol otherwise had no way for a member to record that they were leaving; §2.7.1 point 2 names it, and self-initiated rotation, among the capability-free entries excluded from branch weight. Previously v1.0. A reference implementation exists (see the repository root); where the two differ, this document is normative and the divergence is recorded in the implementation.
...
Each **person** (not each device, not each network) holds one identity per network they belong to. **How those keypairs come into existence is a conformant implementation's choice between two models**, and this section specifies both because they trade against each other rather than one being correct:

- **Derived.** A single high-entropy **master seed**, generated once and stored locally (analogous to a BIP-39-style mnemonic, supporting deterministic child key derivation such as BIP-32), from which every per-network keypair is derived as §1.2 describes.
- **Independent.** A distinct high-entropy seed generated per network, with no seed above them and no derivation relating one to another.

Whichever model an implementation chooses, every seed it holds is:
- Never transmitted over the network, ever, under any circumstance.
- The only thing from which that identity's keys can be reproduced.

**The trade, stated in both directions, because a specification naming only one model leaves a conformant client unable to choose the other.** Derivation buys recovery from a single phrase: one secret restores every identity its holder has. Independence buys a blast radius of one network: one leaked seed exposes one membership rather than all of them — and under a master seed the object with the largest blast radius is precisely the one a user is told to write down and keep.

**Recovery costs are not symmetric, and neither model recovers as much as "recovery" suggests.** A seed reproduces an *identity*; it is not data and it is not a network. Under either model, **a phrase alone restores nothing**, because a network's identifier cannot be derived from a seed — returning to a network requires the seed, that network's id, and an address at which to reach it. What differs is the shape of a backup:

- Under a **master seed**, a backup is one phrase plus the set of network ids and addresses.
- Under **independent seeds**, a backup is a *set* — one seed per network, with its id and address — which is a bundle an implementation exports rather than something a person transcribes. An implementation choosing this model MUST provide such an export, because there is otherwise no path by which an identity survives the loss of the device holding it.

Neither model provides recovery from losing the backup itself. There is no recovery service and no authority that can reissue an identity; a member who has lost their seed and their backup is, to every network they belonged to, a different person.

### 1.2 Per-Network Identity

For every network a user joins there is a **distinct keypair**, used for nothing else and shared with no other network.

**These are requirements on the result, not on the mechanism.** An implementation satisfies this section by exhibiting the properties below, however its keypairs are produced:
...
- **Reproducibility:** An identity survives the loss of the device holding it, given whatever §1.1 says a backup consists of under the chosen model. Within one network, the same seed always yields the same keypair, so a recovered member returns as themselves rather than as a stranger the governance log has never seen.
- **Provable common ownership, at the user's discretion:** The user must be able to *choose* to prove that two per-network identities are the same person (e.g. to their own alt accounts, or to a trusted party), without this being derivable by anyone else. (Mechanism: a signed statement linking two per-network public keys, only created and shared voluntarily.)
  ...
  **That statement must have a serialized form of its own**, and this is stated because it was missed once already one layer over. "Shared voluntarily" is half the mechanism: a proof that can be constructed and verified inside one process but not *handed to anybody* describes the far end of the journey rather than the journey, which is exactly what §5.6 records about invites — their bytes existed only inside the join request that presents them, so issuing one and having no way to send it was possible right up until something tried. An implementation therefore owes the proof an encoding, domain-separated from the statement its two sides sign so the signed bytes cannot be presented as the carrier.

  **Decoding one MUST verify both signatures**, since a decoded-but-unverified proof is indistinguishable from a proven one at every later call site. And an implementation MUST NOT stop there: a valid proof says *some* pair is commonly owned, so a recipient has to compare the pair it names against the pair it expected. The interesting forgery is not a broken signature — it is two genuine signatures over a true statement about somebody else.

**How each model reaches them.** Under a master seed, the keypair is derived deterministically from the seed and the network's identifier:

```
network_identity_keypair = derive(master_seed, network_id, "identity")
```

so unlinkability rests on the derivation function hiding its inputs, and reproducibility on possession of the one seed. Under independent per-network seeds, each keypair is generated from its own entropy, so unlinkability holds **by construction** — two independent random keypairs are uncorrelatable without assuming anything about a derivation function at all, which is the weaker requirement of the two — and reproducibility rests on possession of that network's seed. Provable common ownership is unaffected either way: it is a voluntary signed statement over two public keys (see the App-layer specs' identity-linking mechanism) and never referenced a seed.
...
A person may want to act as the same identity from multiple physical devices (a laptop and a phone, say), without any device other than a deliberately-chosen few ever holding seed material itself — copying seeds to every device they are used from would mean losing or compromising *any one* of them forces rotating the identities those seeds cover, which is exactly the outsized blast radius this design is meant to avoid. Under §1.1's derived model that is every network at once; under independent seeds it is every network whose seed that device holds.

**Devices are independent, then linked — never derived from an identity's seed.**

1. **Each device generates its own local device seed** at setup, independently, never leaving that device and never shared with an identity's own seed material or with any other device. A device is not a derivative of an identity's seed; it starts as its own thing and is subsequently *authorized* to act on that identity's behalf.
2. **Per-network device keys, one per network.** A device derives a distinct device keypair per network from its own local device seed + network_id — for the identical unlinkability reason §1.2 requires identities to be distinct per network: a device's participation in one network should not be correlatable, by its key material alone, to its participation in another. A device seed is a *device's* secret rather than an identity's, so this stays a derivation regardless of which model §1.1 the identity uses; an implementation may equally generate device keypairs independently per network, which satisfies the same requirement by construction.
3. **Linking (enrollment) requires that network's identity key, once, per network.** To authorize a device for a given network, whichever device can produce that network's per-network identity private key — by deriving it from a master seed, or by holding that network's own seed, per §1.1 — does so transiently, in memory, as it already does for any per-network operation, and signs a **device certificate**: `(network_id, per_network_identity_pubkey, device_pubkey_for_that_network, device_label, issued_at)`. This certificate is gossiped and recorded in that network's governance log (§2.7) — reusing the same tamper-evident, independently-replayable structure rather than inventing a parallel record type, even though device linking isn't a group/capability action.
...
5. **No protocol-level "primary device" role.** Any device that can produce a network's identity key can mint or revoke device certificates for that network — the protocol has no concept of one designated primary device, only "devices that can produce the identity key" versus "devices that hold only a certified device key." How many devices a person chooses to trust with seed material (ideally as few as possible) is an operational choice, not a protocol distinction. Note that the two models of §1.1 differ in reach here: a device holding a master seed can enroll devices for *every* network that identity belongs to, whereas a device holding one network's seed can do so only for that network.
...
8. **Recovering from losing every seed-holding device is a separate concern**, already covered by §1.1 — not something this section needs to re-solve. Once seed material is recovered onto any device, that device can immediately mint or revoke certificates for the networks it can produce identity keys for, since certificate authority was never tied to a specific device, only to possession of the identity key.
...
### 2.5.1 Self-Removal — Leaving a Network

**A `MembershipChange` whose `action` is `Remove` and whose `identity` is the entry's own author requires no capability.** It requires only that the author is a current member of the network, the same floor §1.3's self-initiated rotation and §2.6.1's vote proposal stand on. Whether the author is in the group named is a question about applying the entry, not about authorizing it, and is answered by the ordinary "not in group" refusal.

**This exists because the protocol otherwise had no concept of leaving at all.** §2.4 covers admission and §2.5 covers revocation, and both are things done *to* a member by somebody holding a capability. Removal is gated on `manage-membership:<group>`, which a departing ordinary member does not hold — so the one member who knows they are leaving was the one member who could not record it. Every other member's replayed state then said the departed identity was still in `everyone`, still in every group it had been put in, and still a leaf whose key material each epoch rotation wraps, indefinitely.

**Why it does not weaken the gate beside it.** The entry is **monotone downward and self-directed**: it grants nothing, it names nobody but its signer, and the signature that proves the entry is the same signature that proves the identity being removed. There is no version of it that escalates a capability and no version of it that touches another member. That makes it a materially different object from the capability-gated `MembershipChange` it shares an encoding with, rather than an exception carved out of one — an implementation refusing a removal that names anybody but the author is enforcing the same rule, not a stricter one.

**A departure records; it does not itself rotate.** The departing member is excluded by the next epoch rotation, and a `revoke-node` holder MAY rotate on observing a departure, but the entry confers no authority to mint a `MemberRevoked` rotation. Letting it would open a grinding path rather than close one: under §2.4's automatic admission a multi-use invite lets one party join, leave, join and leave, and each cycle would force a real rotation and a replayed governance entry. Keeping the rotation on the capability path leaves the cost with the side that already bears it, and leaves the guarantee intact — a departed member who kept their keys is exactly a member who has not been revoked yet, which is the state revocation already exists to resolve.

**Leaving the network is one entry per group, not one entry.** A removal names a single group, as every membership change does, so a member who is in `everyone` and two roles writes three of them. This document does not make removal from `everyone` imply removal from the rest: the two are already distinguishable — a member removed from a role is still a member — and inventing an implicit cascade here would differ from what §2.5's explicit cascade does, which is the one place membership removal is allowed to reach beyond the identity it names.

**It is a statement, not a request.** The removal takes effect on replay like any other entry. A design in which leaving required an approval would mean a member could not leave a network whose capability holders had all gone, which is the state a person is most likely to want to leave.

**It says nothing about whether the departing member kept their keys, deliberately.** Re-admission after a self-removal is ordinary admission: the log records added, removed, added, and everything the identity wrote stays attributed to it. A consuming application may destroy the seed as it departs, which makes return impossible by construction rather than by rule — but that is that application's product decision, and a specification assuming it would write one client's choice into the wire. Anything stronger would be a ban, and this protocol has no such concept; building one in as a side effect of leaving would be a larger change than this section, and nothing has asked for it.

**Ordering, for implementers.** The entry is signed by the key being departed with, so where a client destroys that key it MUST write and publish the departure first. A node that is offline or has no route cannot announce anything, so leaving is best-effort at the transport layer and an interface MUST distinguish a published departure from an unpublished one rather than reporting success either way.
...
#### 3.5.1 A Repeated Request Replaces a Leaf — Never Adds a Second One

**Previously unspecified, and found by a bug rather than by review.** Key delivery is a request and a response, so it can fail the way every request can: the request is lost, the responder is busy, or the response never arrives. Nothing in §3.5 said what happens when the requester asks again, and the answer an implementation reaches naturally — run the same add again — is unsafe in a way that stays invisible for a long time.

**A request that is never answered strands a member permanently.** They hold an identity the log records, honest nodes will serve them content, and they hold no key with which to open any of it. There is no other mechanism by which they can obtain one, so an implementation MUST make asking again possible. That is the requirement this section exists to create.

**Answering a repeat request by adding the requester again is forbidden.** An identity already occupying a leaf that is added a second time occupies two, and nothing about that looks wrong at the time: they receive a Welcome, they derive the current key, every member agrees on the epoch, and the rotation extends the log rather than forking it. It surfaces at revocation, because removal is expressed against an *identity* while MLS removes a *leaf* — so an implementation resolving one to the other finds the first leaf holding that credential, removes the abandoned one, rekeys, and delivers the new key to the member it was asked to exclude, who is still sitting on the leaf nobody removed. **The removal reports success and the guarantee in §3.1 is gone.** An implementation MUST NOT be able to reach that state.

**Delivering the epoch key alone is also insufficient**, and this is the more tempting mistake because it is cheap and appends nothing. A requester asking again is by construction a requester with no group state: they are asking because they hold no key, and they hold no key because the Welcome that would have carried both never arrived. Keys alone leave them able to read what exists now and unable to process any later commit, so they fall out of the group at the next membership change — silently, having looked recovered. A response that restores less than the ability to follow the group is a partial recovery presented as a whole one.

**So: an implementation MUST answer a repeated request from an identity already in the group by replacing that identity's leaf** — removing the stale leaf and adding the requester's new key package **in a single commit**, which produces one rotation and a Welcome the requester can open. This is also the honest record of what happened, which matters in a log every member replays: that member's leaf was replaced.

Three consequences worth stating rather than leaving to be discovered:

- **The rotation is an ordinary membership rotation** and requires whatever capability admitting a member requires. A replacement widens nothing — the roster before and after is identical — so it needs no capability of its own.
- **Idempotent does not mean free.** Each replacement is a real rotation and writes a governance entry every node replays. An implementation MUST therefore leave *whether* to answer as a decision its caller makes, rather than answering automatically, so that a peer asking in a loop is bounded by the same judgement that bounds admission.
- **The requester may re-send the key package it sent the first time**, and an implementation MUST accept it. A node that never completed a join still holds the pending key package it generated, and requiring a fresh one would mean the retry path depended on state the failure may well have destroyed.
...
- **Kademlia DHT** for WAN peer/content routing at scale. **A routing table must be built, not merely filled in.** A node learns addresses for peers it is already connected to; if that is all it ever does, its table is one hop deep and a provider query can only ask peers already on the other end of a socket — content held two hops away is unfindable, and the symptom is indistinguishable from that content genuinely having no providers. A node with discovery MUST therefore bootstrap its routing table (a query for its own key, which returns the peers closest to it and populates the table on the way) once it has a peer to walk from, and periodically thereafter, since a table that is never refreshed thins out as peers leave until it can no longer route. **This is what makes a network of many members work without every member connecting to every other one**: content is fetched from whichever holder answers (Storage Spec §3), not from its author, and the DHT is what finds one.
...
- **Direct member-to-member delivery**, `/intranet/direct/1.0.0`, carried for consuming specs that need to hand one member a payload rather than publish it — Chat Application Spec §6.2 is the first, delivering the invite to a conversation network inside a network the two already share.

  **It is generic, and it is worth saying why rather than only that.** It was requested as `/chat/dm-invite/1.0.0` (spec 07 §7, E10), and adding it under that name would have put an application's vocabulary in this document's transport stack — which §0 rules out in as many words. So it carries a **namespace, a kind, and an opaque payload**, exactly as §2.7.2's application entry and §2.6.2's policy map do, and a second consumer needs no second protocol. That is now the established answer to a consuming spec asking for a mechanism: the platform gains a door, not a room.

  **What it is, and is not, for.** It moves a *message*, so unlike everything else here it is not pull-based and carries no history: there is nothing to catch up on, because a payload whose value is having reached a particular person now is not state a partitioned node needs late. It follows that **nothing is stored by either party**, and an implementation MUST NOT queue undelivered payloads on a third node — a store-and-forward inbox would put one member's private request on somebody else's disk, which is the outcome a consuming spec choosing direct delivery is choosing against. Delivery requires the recipient reachable, and retrying is the sender's.

  **Payloads MUST be bounded**, and the bound belongs well below what content addressing carries. This is not a transfer path: bulk belongs behind a CID that ordinary swarm serving can resolve (Storage §4), and a carrier permitted to grow into one would be a second content path with none of that section's source selection, backpressure or verification.

  Three obligations on an implementation, and the second is the one a signature alone does not meet:

  1. **A payload MUST be signed by the sending identity**, over the sender, the namespace, the kind and the payload together — so none of them can be altered in flight, and none can be lifted into a message naming a different sender or aimed at a different consumer.
  2. **The claimed sender MUST be checked against the connection the payload arrived on.** A signature proves the named identity composed those bytes; it does not prove that whoever delivered them is that identity, and it *travels* — so anybody who has ever received one can replay it. This is the same check §5.3 requires of a chunk request and Real-Time §2.2.1 of a media frame, and without it a peer can present another member's message as its own.
  3. **Delivery MUST be metered per sending identity**, never per peer id, for the reason §5.3 gives: a peer id is free to regenerate, so a limit keyed on one is bypassable by anybody willing to make a keypair. A carrier without this is a spam channel, and a consuming spec cannot add the limit afterwards because an inbound payload reaches the node before any consumer has decided anything.
  4. **The acknowledgement MUST be reported to the sender.** This is stated because saying that an acknowledgement *exists* turned out not to say that anybody can see one: the reference implementation received the response, and its event loop had no arm for it, so the ack was dropped one layer below the consumer that needed it. What that leaves a sending consumer is a choice between re-sending forever and forgetting after one attempt, and this carrier is the layer that must not queue (below) — so the retry is the sender's and it needs to know when to stop. **Found by the first consumer to send something**, which is now the third time this section's mechanism has been completed that way, after §1.2's proof and §5.6's invite bytes; the pattern is that a mechanism specified from the receiving end is specified halfway.

  **What the transport does not check, deliberately.** It does not decode the payload, does not know whether the sender is still a member, and does not know whether the recipient wants to hear from them. All three are the consuming spec's, and the middle one is worth naming because it is tempting: this layer holds no governance state at rest for the purpose, so a node wanting *only from a current member* replays its own log and refuses. An acknowledgement is therefore **delivery-level only** — it says a payload reached a consumer that understands it, never that anybody agreed to it, because a request/response cannot wait on a person reading their screen.
...
3. **There is no third tier.** If hole-punching also fails (common with symmetric NAT or certain CGNAT configurations), **the two peers do not connect.** The relayed circuit opened in step 2 exists to carry the DCUtR negotiation and nothing else; when the upgrade fails, that circuit MUST be closed and MUST NOT be used to carry protocol or application payload of any kind — no governance entries, no ledger, no epoch keys, no records, no pointers, no segments, no live payloads.

   **A failed negotiation does not announce itself, so the rule has to be enforced on a deadline rather than on an event.** This is the part an implementation will get wrong by doing exactly what the sentence above appears to ask: closing the circuit when the upgrade *reports* failure. DCUtR reports a failure only when its own direct dial fails in a way it observes; when that dial fails at the transport level — a timeout, which is the ordinary outcome behind a symmetric NAT — the attempt is simply abandoned and nothing is emitted at all. A node waiting to be told therefore waits forever, while the circuit stays open and carries whatever the node sends next. **An implementation MUST therefore bound the time a relayed connection may remain un-upgraded and close it when that bound passes**, independently of any failure signal. The bound belongs between a negotiation's realistic duration and the relay's own circuit ceiling (§5.3), so that a conformant client closes its own circuit before the relay has to, and a slow-but-succeeding punch is not cut off. This was found by a conformance run rather than by review: an entire NAT matrix observed relayed connections surviving indefinitely with no failure event emitted once.

   **Where several addresses name the same relay, an unreachable one must not be attempted first.** Found in a real deployment rather than by review, and the shape is worth stating because it is not the obvious failure. A node holding a reservation publishes a circuit address per address the relay announced, so an invite routinely carries several — and a relay deployed behind a proxy without a public address configured announces its private container addresses beside its public name. Those extra addresses would be merely wasteful if the attempts were independent. They are not: every circuit through one relay shares the connection to that relay, so a dial to an unreachable hop can leave the relay peer abandoned and **cancel the pending requests behind it**, including the one that would have worked. The failure then arrives against the *good* address, reported as a cancelled request rather than as a network error, which points an implementer at the wrong half. An implementation SHOULD therefore order circuit candidates so that a relay hop it cannot route to is attempted after one it can — demoting rather than discarding, since a relay on the same LAN as both peers is reachable and legitimate. This is ordering *within* tier 3 and does not disturb the tier sequence above.

   **A relay is signaling, not transport, and this is a hard rule rather than a budget.** An earlier version of this section described the circuit as "sustained ... for the duration of the session", and a later one kept it as a capped fallback — "a correctness guarantee, not a usable path". Both were wrong in the same direction, and the second was worse for sounding careful: *not a usable path* means not usable, at all, not merely not usable for long. A relay that carries payload — even briefly, even within a ceiling — is infrastructure the network runs on, and acquires exactly the cost, permanence and takedown surface this design exists to avoid (§5.5). The ceilings in §5.3 are defence in depth behind this rule, not the rule itself.

   **A pair that cannot hole-punch reaches each other over IPv6, or not at all.** This is the direct consequence of tier 1's IPv6-first ordering: a pair behind CGNAT typically cannot traverse IPv4 at all, but globally-routable IPv6 addresses need no traversal in the first place. A deployment that expects CGNAT users to depend on relayed circuits for ordinary traffic has misread this design, and now finds it refused rather than throttled.

   **Two members who cannot connect to each other are not partitioned from the network, only from each other.** Everything this protocol carries is pull-based and content-addressed — the governance log (§2), the capability ledger (§4.2), author logs and their pointers — so two members with no mutual path still converge through **any member both can reach**. That member is not relaying: it is an ordinary peer that each of them syncs with, and convergence is a property of the data rather than of the route. A two-member network where neither can reach the other is the one case with no remedy, and is a genuine limitation stated plainly rather than papered over with a relay.

   **This constrains joining too.** An invite (§5.6) carries addresses, and a joiner reaches them through the same three steps. A joiner who cannot establish a direct or hole-punched connection to a member does not join — the circuit that carried the negotiation is not a way in.
...
Any node offering `relay_bootstrap_willing` (§4.2) — including the network's initial bootstrap node (§5.5) — should enforce concrete resource limits, both to protect itself from abuse and to keep its behavior predictable. Reasonable baseline defaults: a cap on total concurrent reservations (e.g. 128), a per-peer reservation cap (e.g. 4), a cap on concurrent relayed circuits (e.g. 32), a maximum circuit duration (e.g. **60 seconds**) forcing periodic re-negotiation rather than indefinitely-held circuits, and a maximum bytes-per-circuit ceiling (e.g. **256KB**) consistent with a relay's job being connection-establishment assistance, not sustained data transport (that's the separate, distinct media-relay role, §4.4).

**The last two figures were 120 seconds and 8MB, and are lowered here deliberately.** Those came from a prior implementation that predated §5.2's prohibition, and were loose enough that a client relaying an entire conversation never met a limit — so the rule held only by clients choosing to obey it, and in practice one did not. A conformant circuit carries a DCUtR negotiation and closes: two small messages and up to three dial attempts, on top of Noise and identify. 60 seconds and 256KB are orders of magnitude above that cost and orders of magnitude below a conversation, which is what converts a rule enforceable only by inspection into one a relay enforces by itself. A relay cannot see what crosses it; it can refuse to carry more than a negotiation's worth.

**The duration is set generously on purpose, because §5.2 made a cut-short negotiation fatal.** A circuit that ends mid-negotiation used to leave the pair on a relayed connection; now it leaves them unable to connect at all. On a lossy mobile link a negotiation can plausibly run ten to fifteen seconds, so the cost of a ceiling a few seconds too tight is two members who cannot talk, and the cost of a generous one is a relay slot held a little longer. **Neither ceiling bounds how large a network can be** — they bound one circuit, so lowering them raises how many introductions a relay can perform per hour rather than lowering it. What bounds a single relay is its concurrent reservation and circuit caps, and what scales past those is more relays and `relay_bootstrap_willing` members (§5.5), never a larger allowance on one host.

**These ceilings are defence in depth behind §5.2's rule, not a budget for payload.** §5.2 forbids a relayed circuit carrying protocol or application payload at all; these limits exist so that a relay stays cheap and disposable even when something misbehaves — a buggy client, a hostile one, or an implementation that has not yet been corrected. They are not a throttle to be raised when users complain, and an implementation hitting the byte ceiling during ordinary operation is not under-provisioned: it is violating §5.2 and should be fixed there. What keeps a bootstrap relay cheap enough to be disposable — small enough for a free hosting tier, stateless enough to replace (§5.4), genuinely optional rather than infrastructure (§5.5) — is that it carries nothing, and the ceilings are what make that true in the presence of bugs.

**A client whose reservation is refused MUST back off before asking again, and this is a requirement on the client rather than advice to it.** Every limit above is a rate or a count, and the reference limiter refills roughly one reservation per two minutes per peer — so a client that re-asks on a fixed short interval spends faster than it earns, empties the bucket, and is then refused *every* time while continuing to ask at the same rate. **The bucket cannot refill, so a transient failure becomes permanent**, and it does so on every affected member at once while ordinary connections keep working and the relay's log keeps showing each peer arriving. Found in a real deployment that way rather than by review: the client asked every twenty seconds, and the retry was what sustained the failure it was trying to clear. An implementation MUST therefore grow the interval between reservation attempts — the reference client doubles from twenty seconds to a ten-minute ceiling and resets on success — and MUST NOT treat a refusal as a reason to retry immediately. The first retry may be prompt, because the case worth answering quickly is a relay that restarted and dropped the reservations it held (§5.4); it is the *repeated* one that has to slow down. This also bounds what a large network does to its own relay: a hundred members each spinning on a failed reservation is a denial of service a network performs on itself, and no relay-side limit can distinguish it from an attack.
...
- **A relay MUST NOT announce an address no remote peer could dial.** A relay promotes the addresses it binds so that its reservations name somewhere, and a host's interfaces routinely include addresses that are useless to anybody else: loopback, and — the one that reaches the field — IPv6 **link-local**, which is meaningless without a scope id and meaningless off the link regardless. The cost is not a wasted entry. An announced address enters every reservation reply, becomes a circuit address each client advertises as its own, and rides into every invite minted from those addresses; clients then dial it for as long as they hold it, taking a handshake timeout each time, against a relay that is working perfectly for everything else. A member without the capability to change the relay set cannot stop it, and the failure names a peer id the network may no longer even use. Private addresses are a different case and **should** still be announced: a relay on the same LAN as its members is genuinely reachable, which is why §5.1.1 demotes an unroutable hop rather than discarding it.
...
**What that dependency is, precisely, given §5.2.** It is a dependency on *rendezvous*: two NATed members need somewhere to meet in order to negotiate a hole punch, and a cached address for a peer behind NAT is not dialable on its own. It is **not** a dependency on the relay carrying anything. A pair that can hole-punch needs the relay for the negotiation and then never again for that session; a pair that cannot hole-punch is not helped by the relay at all, because §5.2 does not permit the circuit to become the path. The remedy for the second case is IPv6, or a member both can reach — never a relay allowance.
...
**An invite must have a serialized form of its own, and that is not a detail.** It is a credential carried to a prospective member *out of band* — pasted into a message, put behind a link, read off a screen — so a representation that only exists inside the join handshake describes the far end of the journey and not the journey. An implementation therefore owes an encoding for the invite alone, domain-separated from the join request that later presents it, so the two cannot be decoded as each other. Decoding establishes framing only: an invite stays a *claim* until it is validated against replayed governance state, and a decoder that verified its signature would invite the reading that decoding had already settled something.

**And that serialized form has a size budget, which follows from the same sentence.** Out of band means a human moves it, so an invite long enough to be truncated, wrapped, or refused by whatever carries it has failed the job this section gives it — the encoding is not merely a representation question. Two things drive the length and neither is obvious from the field list. The first is that a node publishes an address per interface per transport, and §5.2's tiers mean an invite carries direct and circuit addresses together; an implementation SHOULD therefore carry the addresses a recipient could actually dial rather than every address the issuer listens on, since an address reachable only from inside the issuer's own network or overlay is not merely wasted length but, for a circuit, actively harmful (§5.2). The second is that those addresses are highly redundant: they name one node, so they share a peer-id suffix, and an encoding that repeats it per address can spend more than half the field on it. Neither is a conformance requirement — an implementation that ignores both still interoperates — but a deployment that ignores both produces invites several thousand characters long, which is how this was found.
...
- Every identity is a per-network-derived keypair off one unlinkable master seed (§1). Additional devices are independently seeded and linked via per-network certificates rather than ever holding the master seed themselves — a lost/stolen device is revoked by invalidating its certificate (no identity rotation needed) plus, if it may have cached decryption material, a self-initiated epoch rekey. **Unlinkability between a person's identities is a property of their keys and nothing more** (§1.2): two per-network public keys cannot be tied together, and an observer positioned to see both connections — a peer in both networks, a relay carrying both, anyone watching the traffic — can still correlate them by address or timing, which §1.2 places explicitly out of scope. A consuming spec may assume the key-layer property and must not describe it as anonymity request (§1.3).
...
- **A key request must be safe to repeat, and a repeat replaces a leaf rather than adding one** (§3.5.1). Delivery is a request/response and can fail like any other, and a request that is never answered strands a member permanently — recorded in the log, served by honest nodes, holding no key that opens any of it — so asking again must be possible. Answering by adding the requester a second time gives one identity two leaves, which looks correct until revocation: removal is expressed against an identity and applied to a leaf, so it removes the abandoned one and hands the new epoch key to the member it was asked to exclude. Delivering the epoch key alone is also insufficient, since a requester asking again has no group state and would be unable to follow any later commit. Consuming specs may assume a conformant implementation restores a repeat requester fully, and may not assume answering is free: each replacement is a real rotation and a governance entry, so whether to answer stays a caller's decision.
...
- Connectivity is libp2p-based with a clear separation between NAT-traversal signaling and sustained media relaying (§4.4, §5.2). **A bootstrap relay carries no payload, ever**: its circuit exists to carry the DCUtR negotiation, is closed when the upgrade fails, and never carries governance, ledger, keys, records or live payloads. **Closing it must be driven by a deadline, not by a failure event** (§5.2): a hole punch that fails at the transport level emits nothing, so an implementation that waits to be told holds the circuit open indefinitely. A pair that can never hole-punch — two behind CGNAT — reaches each other over IPv6 or not at all; they remain able to converge through any member both can reach, since everything here is pull-based and content-addressed. These are different node roles even when the same physical node fills both. **Unlinkability requires a distinct libp2p PeerId per network** (§1.2), derived the same way as the per-network identity keypair — key-level unlinkability alone is void if the transport layer reuses one fingerprint everywhere. IP-level/timing correlation remains explicitly out of scope, stated honestly rather than silently assumed away.
