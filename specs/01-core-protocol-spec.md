...
**Document status:** v1.1 — stable. §2.5.1 adds **self-removal**: a membership removal naming its own author needs no capability, because the protocol otherwise had no way for a member to record that they were leaving; §2.7.1 point 2 names it, and self-initiated rotation, among the capability-free entries excluded from branch weight. Previously v1.0. A reference implementation exists (see the repository root); where the two differ, this document is normative and the divergence is recorded in the implementation.
