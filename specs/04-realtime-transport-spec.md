...
### 2.2.2 A Relay Must Be Able to Refuse — Previously Unspecified

Core Protocol Spec §4.2 has a node declare what it will contribute, including a `bandwidth_cap`, and §4.3 makes that declaration opt-in and revocable per network. Every *consumer* of the capability ledger reads it — relay selection (§2.3) and swarm source selection both weight it, and a node advertising no upload capacity is not offered work at all. **What no section said is that the declaring node enforces anything against itself**, and an implementation can therefore satisfy every requirement in this document while treating its own advertisement as a fact about itself that only other people act on.

That was survivable while a relay forwarded one envelope for each one it received: the cost was bounded by what senders could push at it. §2.2.1 changes the arithmetic, because one accepted envelope now costs N−1 sends. The distance between what a node promised and what it can be asked to do became a multiplier, so the promise has to bind.

**A media relay implementation is therefore required to bound, at minimum:**

1. **Concurrent calls carried.** Media relaying is sustained for the life of a call, unlike a bootstrap circuit which Core §5.3 caps at two minutes and eight megabytes. A handful of simultaneous calls is a far larger commitment than the same number of circuits, and nothing else in this document bounds it.
2. **Participants in any one carried call.** This is the fan-out factor — the multiplier applied to every frame — so it is the ceiling that bounds what a single envelope can cost.
3. **Sustained bytes forwarded per unit time**, charged against **what leaves the node**: frame size times recipient count. Charging the inbound size instead under-meters by exactly the factor §2.2.1 introduced, which would make the ceiling decorative in the one case it exists for.

Three properties these ceilings must have:

- **Refusal is a first-class outcome, not an error path.** A relay at its limit declines to carry a call, and the call selects another through the ordinary mechanism (§1.4, §2.4) — the same path taken when a relay goes offline. Nothing about a refusal is exceptional, and participants must not treat it as a call failure.
- **Refusals are observable at the refusing node.** A limit that surfaces only as a peer's degraded experience is indistinguishable from one that never ran, which is the failure Reference Test Harness Spec §2.5 already requires a test for on the bootstrap relay's limits. Sustained refusal is also the only signal a node's operator gets that they volunteered more than they have.
- **A partially served frame is worse than a refused one.** A relay that forwards to some participants and not others when it runs short converts a bandwidth ceiling into silent, one-sided call degradation — harder to diagnose and worse to experience than a clean refusal. A fan-out that does not fit is refused whole.

**These values are deliberately unspecified, and deliberately not network policy.** A ceiling is a statement about one node's hardware and connection, so a network that could set it would be able to compel a member to spend bandwidth it never offered — the exact inversion of §4.3's opt-in contribution model. The natural default is the node's own advertised `bandwidth_cap`, which is the number it already published and the one other nodes are already planning around.
...
- A media relay **enforces its own advertised limits and may refuse** (§2.2.2) — declining a call, or a frame beyond its byte allowance, is ordinary rather than exceptional, and the call renegotiates onto another relay exactly as it would if this one went offline. Consuming specs must not treat a refusal as a call failure.
