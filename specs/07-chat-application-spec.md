...
| **E15** | **Landed in Core v1.2.** §1.1–§1.3 now admit two identity models — a master seed with derived per-network keypairs, and independent per-network seeds — and §1.2 states its properties as requirements on the *result* rather than as consequences of the derivation, which independent generation satisfies by other means (unlinkability by construction, which is the weaker assumption). §1.1 states the trade in both directions and what recovery costs under each, including that a phrase alone restores nothing either way. The harness spec §5 was amended alongside, since a conformance test that exercised only the derived model would have failed a conformant client | Core §1.1–§1.3, Harness §5 |
| **E16** | ✅ **Implemented.** **Self-removal from a group.** A member could not say they were leaving: `MembershipChange { action: Remove }` is the entry for it and was gated on `manage-membership`, which a departing ordinary member does not hold — so the one member who knows they are going was the one member who could not record it. The rule that landed: **a `MembershipChange` removing the entry's own author is valid without a capability**, requiring only current membership. It is safe because the entry is monotone downward and self-directed — it grants nothing, names nobody but its signer, and its signature is the same one that proves the identity it removes — so no version of it escalates and no version of it touches another member. A departure **records and does not itself rotate**: `RotationReason::MemberRevoked` stays `revoke-node`'s to trigger, because under `admission: auto` a multi-use invite would otherwise let one identity join and leave repeatedly, forcing a real rotation each time; the next rotation excludes them, and a `revoke-node` holder may rotate on seeing a departure. It is a statement rather than a request, or a member could not leave a network whose capability holders had all gone. And it says nothing about whether the departing member kept their keys — re-admission is ordinary admission, and a spec that assumed the seed went with the departure would write one client's product decision into the wire. **Excluded from fork-choice branch weight** (§2.7.1 point 2), which the proposal did not anticipate: an entry needing no capability must carry no weight, and under `admission: auto` a departure is the cheapest such entry to grind. Specified in Core §2.5.1 | Core §2.5.1 |
...
**E16 was the only item here that blocked a whole verb rather than a feature**, and until it
landed there was no leaving a network at all: the reference client's `forget` deletes its own store and seed,
...
remedy and the person who knows is the person leaving. Core §2.5.1 settles the platform half;
what a consuming client still owes is publishing the departure *before* it destroys anything,
and saying which of the two happened when it has no route.
