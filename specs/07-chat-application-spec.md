...
**Document status:** v0.4 — draft. §7 adds **E16**, self-removal from a group: a member cannot record their own departure, because every membership change is gated on `revoke-node`. Previously v0.3 — §1.6 fixes sidebar order, §1.7 a network's name and §1.8 categories as named, ordered metadata over a scope that already exists; `SetPosition` is channel-update `0x07` and categories are entry kinds `0x05`/`0x06` (§3.8). All of it is implemented in `ko-ls` as of 2026-08-23. A reference implementation is in progress (`ko-ls`); where the two differ, this document is normative and the divergence is recorded in the implementation.
...
| **E14** | ✅ **Implemented.** **A repeated key request replaces a leaf rather than adding one.** Delivery is a request/response and can fail like any other, and nothing said what happens when the requester asks again — while a request that is never answered strands a member permanently. Answering by re-adding gives one identity two leaves, which surfaces only at revocation, where removal resolves an identity to its *first* leaf and therefore excludes nobody. Specified in Core §3.5.1 | Core §3.5.1 |
| **E15** | **Independent per-network seeds.** Core §1.1 specifies one master seed per person with each network identity derived from it; the reference client generates independent entropy per network instead, so that no single secret links every identity a member holds. Unlinkability, determinism within a network and voluntary common ownership all still hold — they are properties of the result rather than of the derivation — but §1.1–§1.3 describe only the derived model, and §1.3's enrollment wording names a master seed the client does not have. Blocks nothing: the platform never sees a seed, only the keypairs it produces | Core §1.1–§1.3 |
| **E16** | **Self-removal from a group.** A member cannot say they are leaving. `MembershipChange { action: Remove }` is the entry for it and is gated on `RevokeNode`, which a departing member almost never holds — so the one member who knows they are going is the one member who cannot record it. The rule asked for: **a `MembershipChange` removing the entry's own author is valid without a capability.** It is safe for a reason worth stating as a rule rather than a special case — the entry is monotone downward and self-directed: it grants nothing, names nobody but its signer, and its signature is the same one that proves the identity it removes, so no version of it escalates and no version of it touches another member. Two things it must settle rather than imply. **A departure records and does not itself rotate**: `RotationReason::MemberRevoked` stays `revoke-node`'s to trigger, because under `admission: auto` a multi-use invite would otherwise let one identity join and leave repeatedly, forcing a real rotation and a replayed entry each time; the next rotation excludes them and a `revoke-node` holder may rotate on seeing a departure. And **it is a statement, not a request** — it takes effect on replay like any other entry, or a member could not leave a network whose admins have all gone, which is the state a person most wants to leave. Says nothing about whether the departing member kept their keys: re-admission after a self-removal is ordinary admission, and a spec that assumed the seed went with the departure would write one client's product decision into the wire | Core §2 |
...
**E16 is the only item here that blocks a whole verb rather than a feature.** Without it there
is no leaving a network at all: the reference client's `forget` deletes its own store and seed,
and to every other member nothing has happened — the departed identity stays in `everyone`,
stays in every role, and stays a leaf the epoch rotates key material for, forever. That is not a
break, since a member who destroyed their seed can read nothing, but it compounds: the group
only grows, the roster reports *gone permanently* as *not connected*, and a seed copied before
the deletion keeps receiving key material for every future epoch, because revocation is the
remedy and the person who knows is the person leaving.
